    ...
    void LoadModules (MAP_GENERATOR::MG generator, std::string path)
    {
        for (auto& p : std::filesystem::directory_iterator(path))
        {
            ...
            if (fileNameSplit[1] != "n")
            {
                ...
                {
                    generator->_loadedWindingModules.emplace_back(MODULE::Module(loadedHeight, loadedEntranceSide, loadedExitSide, loadedParkourDifficulty, moduleType, fileName, 0, 0, json));
                }
                else
                {
                    generator->_loadedStraightModules.emplace_back(MODULE::Module(loadedHeight, loadedEntranceSide, loadedExitSide, loadedParkourDifficulty, moduleType, fileName, 0, 0, json));
                }
...
//                    module.entranceSide += 1;
//                    module.entranceSide %= 4;
//                    module.exitSide += 1;
//                    module.exitSide %= 4;
                    lastExitSide += 1;
                    lastExitSide %= 4;
