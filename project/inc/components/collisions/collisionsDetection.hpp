...
void CheckOBBCollisionsOne(const COLLIDER::Collider& singleCollider, COLLIDER::ColliderGroup B, std::unordered_map<COLLIDER::ColliderGroup, COLLIDER::Collider*> colliders, std::unordered_map<COLLIDER::ColliderGroup, u64> collidersCount, glm::vec3& overlapVector)
{
    for (int j = 0; j < collidersCount[B]; j++)
    {
        if (colliders[B][j].local.group == B)
        {
            //std::cout << collidersCount[B] << std::endl;
            auto& c1 = singleCollider;
            auto& c2 = colliders[B][j];
            ...
            // If all true - collision detected
            {
                ...
                std::cout << "boom " << overlapVector.x << " " << overlapVector.y << " " << overlapVector.z;
            }
        }
    }
}
